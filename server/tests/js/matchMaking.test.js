...
describe("Works with a 2^(2) case with single qualifying matches (between 9 to 12 (inclusive))", () => {
    ...
    describe("Match tests", () => {
        ...
        test("Matches have previousMatchId property correctly assigned where needed", () => {
            expect(result[2][2].previousMatchId).toContain(result[3][0]._id); 
            expect(result[2][0].previousMatchId).toContain(result[3][3]._id); 
        });
    });
...
describe("Works with a 2^(2) case with double qualifying matches (between 13 and 15 (inclusive))", () => {
    ...
    describe("Match tests", () => {
        ...
        test("Matches have correct nextMatchIds (spot checks)", () => {
            ...
            expect(result[3][5].nextMatchId).toBe(result[2][1]._id);  
        });
        test("Matches have previousMatchId property correctly assigned where needed", () => {
            expect(result[2][2].previousMatchId).toContain(result[3][0]._id); 
            expect(result[2][2].previousMatchId).toContain(result[3][4]._id); 
        });
    });
...
describe("Works with a 2^(3) case with single qualifying matches (between 17 to 24 (inclusive))", () => {
    ...
    describe("Match tests", () => {
        ...
        test("Matches have previousMatchId property correctly assigned where needed", () => {
            expect(result[3][4].previousMatchId).toContain(result[4][0]._id);
            expect(result[3][5].previousMatchId).toContain(result[4][2]._id);
        });
    });
    ...
    describe("Participants tests", () => {
        ...
        test("Participants for qualifying matches are correct (spot checks)", () => {
            expect(result.at(-1)[0].participants).toContain(9);
            expect(result.at(-1)[0].participants).toContain(12);
            expect(result.at(-1)[1].participants).toContain(13);
            expect(result.at(-1)[2].participants).toContain(11);
        });
        test("Participants for round matches are correct", () => {
            expect(result.at(-2)[0].participants).toContain(1);
...
describe("Works with a 2^(3) case with double qualifying matches (between 25 to 31 (inclusive))", () => {
    ...
    describe("Match tests", () => {
        test("Number of qualifying matches is correct", () => {
            expect(result.at(-1)).toHaveLength(9);
        });
        test("Number of MATCHES is correct", () => {
            expect(result.flat()).toHaveLength(teams.length - 1);
        });
        ...
        test("Matches have previousMatchId property correctly assigned where needed", () => {
            expect(result[3][4].previousMatchId).toContain(result[4][0]._id);
            expect(result[3][4].previousMatchId).toContain(result[4][8]._id);
            expect(result[3][5].previousMatchId).toContain(result[4][2]._id);
        });
    });
    ...
    describe("Participant tests", () => {
        ...
        test("Participants for qualifying matches are correct (spot check)", () => {
            ...
            expect(result.at(-1)[3].participants).toContain(11);
            expect(result.at(-1)[7].participants).toContain(15);
            expect(result.at(-1)[5].participants).toContain(13);
        });
        test("Participants for round matches are correct", () => {
            expect(result.at(-2)[0].participants).toContain(1);
            expect(result.at(-2)[7].participants).toContain(2);
